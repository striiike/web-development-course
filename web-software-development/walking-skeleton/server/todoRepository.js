...
const deleteById = async (userId, id) => {
    const result = await sql`DELETE FROM todos WHERE id = ${id} AND user_id = ${userId} RETURNING *`;
...
const findById = async (userId, id) => {
    const result = await sql`SELECT * FROM todos WHERE id = ${id} AND user_id = ${userId}`;
